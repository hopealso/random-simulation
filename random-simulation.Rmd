...
Examining the empirical density of our sample as diagrammed on our histogram, we can see that the central tendency of our 10 bins has a mean of 1 and further is within one standard deviation of 1. Thus the mean and central tendency of these bins are consistent with a continuous uniform distribution on [0,1] and our goal.
...
For a uniform distribution we expect that the center of our data will be midway through the range of the distribution. Thus the sample mean of 0.516 is quite close to our expected theoretical mean of 0.5 (midway between 0 and 1, or $\frac{(1-0)}{2}$. We expect that the variance of our data will be $\frac{(b-a)^{2}}{12}$, or 1/12, and our sample mean of 0.084 is quite close to this.
...
* Examining the spread of the histogram's bins through calculating their moments
* Examining the interquartile range of the data (IQR)

If we define **"estimating a distribution"** as estimating, for a given data set, both its distribution (which distribution the data appears to take) and its parameters (the parameters for the distribution we suspect), which of these methods is most helpful?

Let's create a set of data with similar parameters which is *not* uniform as a basis of comparison.

```{r q2-1-1-compare}
sample.norm <- rnorm(1000, mean = 0.5, sd = 1/12)
summary(sample.norm)
hist.norm <- hist(sample.norm, main = "Normal Sample")

hist.norm.mean <- mean(hist.norm$density)
hist.norm.sd <- sd(hist.norm$density)

# Plot our histogram again, adding mean and standard deviation to assist in interpreting spread.
plot(hist.norm, freq = FALSE, main = "Spread of Normal Sample")
abline(h = hist.norm.mean)
abline(h = hist.norm.mean + 1.96 * hist.norm.sd, col = "red", lty = 2)
abline(h = hist.norm.mean - 1.96 * hist.norm.sd, col = "red", lty = 2)
```

*How can we confirm that data is uniform?*

For any uniform distribution we know that the frequency and the density of its values should be uniform across its range. 
