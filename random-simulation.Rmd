...
However, if we know the minimum, maximum, mean and standard deviation of the sample, it seems we can come close to estimating the minimum and maximum of its population distribution. We may need to make some assumptions regarding just how far extended from the bounds of our sample the distribution in question may be.
...
title.traffic.1 <- "100 Random Numbers Row-Transformed from Binary Traffic Flow Sequence"
hist.traffic.1 <- hist(sample.traffic.1, main = title.traffic.1)
#hist.traffic.1

title.traffic.2 <- "100 Random Numbers Column-Transformed from Binary Traffic Flow Sequence"
hist.traffic.2 <- hist(sample.traffic.2, main = title.traffic.2)
#hist.traffic.2
...
For a uniform distribution we expect that the center of our data will be midway through the range of the distribution. Thus the sample mean of `r sample.traffic.2.mean` is quite close to our expected theoretical mean of 0.5 (midway between 0 and 1, or $\frac{(1-0)}{2}$. We expect that the variance of our data will be $\frac{(b-a)^{2}}{12}$, or 1/12, and our sample variance of `r sample.traffic.2.var` is relatively close to this.

***Overall Perception***

It appears that our data is very nearly random, yet both transformations contain an outlier value that pulls the data outside its expected uniform density and pulls the moments slightly askew of our expectations for a uniform distribution. 
